...
create_dockerfile <- function(dockerfile = "docker/Dockerfile", rocker_name = "rstudio", include_python = TRUE){
  ...
  cli::cli_alert_info("Using Default Image: {.val {rocker_image}}")
...
setup_packages <- function(packages){
  ...
  set_package_preference <- match.arg(packages, c("loaded", "none", "installed"))

  if(set_package_preference %in% "none"){
    reset_dir()
    create_directories()
    return(bash_file)
  } else if(set_package_preference %in% c("loaded", "installed")){

    cli::cli_h1("Setting Package Preferences")
    create_directories()
    package_df <- sessioninfo::package_info(pkgs = set_package_preference)
...
build_image <- function(image_name = "image_name", dockerfile = "docker/Dockerfile"){
  ...
  if(is.null(image_name)) {
    name_set <- tolower(basename(rstudioapi::getActiveProject()))
  } else {
    name_set <- tolower(name)
  }
...
dockerfile <- R6::R6Class(classname = "dockerfile",
